...
CRIME_LEGAL_MAP = {
    ...
    "card_skimming_pos_fraud": {
        "label": "Card Skimming, ATM Cloning & POS Fraud",
        "scope": "Citizen",
        "bns": ["303", "318", "112"],
        "it_act": ["66C", "66D"],
        "emergency_steps": [
            "Hotlist/block your debit/credit card immediately via your bank mobile app or customer care.",
            "Reporting unauthorized transactions within 3 days grants ZERO customer liability under RBI circular DBR.No.Leg.BC.78/09.07.005/2017-18.",
            "Lodge a written dispute at your home branch with the ATM/POS location details.",
            "Call 1930 immediately to register the unauthorized withdrawal on the cybercrime portal."
        ]
    },
    "utility_bill_fraud": {
        "label": "Electricity / Utility Disconnection Phishing",
        "scope": "Citizen",
        "bns": ["318", "319", "336"],
        "it_act": ["66D", "43"],
        "emergency_steps": [
            "State electricity boards (DISCOMs) NEVER threaten same-day disconnection from personal mobile numbers.",
            "If an APK was downloaded, turn on Airplane Mode immediately to terminate remote screen-sharing tools (AnyDesk/RustDesk).",
            "Uninstall untrusted apps and call your bank immediately to freeze NetBanking and UPI.",
            "Call 1930 and file a complaint on cybercrime.gov.in."
        ]
    },
    "loan_app_fraud": {
        "label": "Predatory Loan App & Recovery Agent Harassment",
        "scope": "Citizen",
        "bns": ["308", "351", "79", "356"],
        "it_act": ["43", "66E", "67"],
        "emergency_steps": [
            "Immediately uninstall the predatory app and revoke contact/storage/camera permissions.",
            "Broadcast a status disclaimer to your phone contacts warning them of fake defaming messages sent by illegal apps.",
            "Lodge an urgent complaint on the RBI Sachet Portal: https://sachet.rbi.org.in.",
            "DO NOT pay extortionate harassment charges — paying invites further threats.",
            "File a formal police complaint under BNS Section 308 (Extortion) & Section 351 (Criminal Intimidation)."
        ]
    },
    "courier_customs_fraud": {
        "label": "Courier / Customs Narcotics Parcel Trap",
        "scope": "Citizen",
        "bns": ["318", "319", "308", "336"],
        "it_act": ["66C", "66D"],
        "emergency_steps": [
            "Hang up immediately! Indian Customs and FedEx NEVER demand online UPI clearance fees.",
            "Do NOT transfer money into any 'clearance escrow' or 'police verification account'.",
            "Call 1930 immediately to report the fraud caller's phone numbers.",
            "Preserve call logs and WhatsApp recordings as digital evidence under Section 63 BSA 2023."
        ]
    },
    "digital_arrest": {
    ...
    "matrimonial_romance_fraud": {
        "label": "Matrimonial / Romance Scam & Honeytrap",
        "scope": "Citizen",
        "bns": ["318", "319", "316"],
        "it_act": ["66D"],
        "emergency_steps": [
            "Stop all money transfers immediately for 'customs gifts stuck at airport' or 'sudden medical emergencies'.",
            "Airport authorities never demand payments into individual bank accounts.",
            "Archive all chat history, audio messages, and matrimonial profiles.",
            "Report the profile to Jeevansathi / Shaadi.com and file at cybercrime.gov.in."
        ]
    },
    "job_visa_fraud": {
        "label": "Fake Job, Overseas Visa & Placement Fraud",
        "scope": "Citizen",
        "bns": ["318", "336", "338"],
        "it_act": ["66D"],
        "emergency_steps": [
            "Stop all further fee payments for fake medicals, visa processing, or security deposits.",
            "Verify company recruitment notices directly on official career portals.",
            "Report suspect beneficiary accounts to 1930 to freeze transactions.",
            "Lodge formal complaint at cybercrime.gov.in."
        ]
    },
    "part_time_job": {
    ...
    "lottery_reward_fraud": {
        "label": "Lottery / KBC / Fake Lucky Draw Scam",
        "scope": "Citizen",
        "bns": ["318", "319"],
        "it_act": ["66D"],
        "emergency_steps": [
            "Real lotteries never demand advance processing fees or GST into individual UPI handles.",
            "Block the fraud caller and report the number on 1930.",
            "Preserve fake prize letters and lottery images for the police FIR."
        ]
    },
    "ecommerce_olx_fraud": {
        "label": "OLX / Fake Army QR Code Scam",
        "scope": "Citizen",
        "bns": ["318", "319", "336"],
        "it_act": ["66D", "66C"],
        "emergency_steps": [
            "REMEMBER: Scanning a QR code ALWAYS DEDUCTS money, never credits money.",
            "Never enter your UPI PIN to receive funds.",
            "Call 1930 immediately to freeze the recipient merchant UPI VPA.",
            "Report the fraudulent seller/buyer profile to the online marketplace."
        ]
    },
    "sextortion": {
    ...
    "deepfake_morphing_harassment": {
        "label": "Deepfake & AI Image Morphing Harassment",
        "scope": "Citizen",
        "bns": ["78", "79", "351", "356"],
        "it_act": ["66E", "67A"],
        "emergency_steps": [
            "Submit an emergency takedown notice under Rule 3(2)(b) of the Information Technology Rules, 2021 (mandatory removal within 24 hours).",
            "Preserve URL links, profile handles, and file hashes as evidence.",
            "Do NOT pay ransom demands to the extortionist.",
            "Lodge complaint on cybercrime.gov.in under Women and Child Cyber Crime cell."
        ]
    },
    "cyber_stalking_harassment": {
        "label": "Cyber Stalking, Bullying & Doxxing",
        "scope": "Citizen",
        "bns": ["78", "79", "351", "352"],
        "it_act": ["66E", "67"],
        "emergency_steps": [
            "Capture timestamped screenshots of stalker profiles, direct messages, and comments before blocking.",
            "Adjust privacy settings on social media to contacts only.",
            "Lodge formal FIR at local Cyber Police Station under BNS Section 78 & IT Act Section 66E.",
            "Request police restraining directions against the suspect."
        ]
    },
    "csam_pocso_offence": {
        "label": "Child Exploitation / CSAM (POCSO)",
        "scope": "Citizen",
        "bns": ["111"],
        "it_act": ["67B"],
        "emergency_steps": [
            "Possession, creation, or transmission of CSAM is a severe non-bailable offence under POCSO Act and IT Act 67B.",
            "Immediately report to National Childline: 1098 and 1930.",
            "Do not recirculate or forward the media; preserve original headers for forensic investigation."
        ]
    },
    "identity_theft": {
    ...
    "hacking": {
        "label": "Account / Device Compromise & Malware",
        "scope": "Citizen",
        "bns": ["316"],
        "it_act": ["66", "43", "66C"],
        "emergency_steps": [
            "Terminate active sessions from account security settings and change passwords from an uninfected device.",
            "Enable hardware-backed or authenticator-based Two-Factor Authentication (2FA).",
            "Disconnect device from internet and run comprehensive malware/spyware scans.",
            "File complaint at cybercrime.gov.in under 'Hacking / Unauthorized Access'."
        ]
    },
    "investment_fraud": {
    ...
    "cyber_terrorism_sabotage": {
        "label": "Cyber Terrorism & Critical Infrastructure Attack",
        "scope": "Corporate",
        "bns": ["111", "197"],
        "it_act": ["66F", "70"],
        "emergency_steps": [
            "Immediate emergency alert to NCIIPC (National Critical Information Infrastructure Protection Centre).",
            "Emergency hot-line escalation to CERT-In (incident@cert-in.org.in).",
            "Sever external routing gateways and activate air-gapped recovery systems.",
            "Preserve immutable cryptographic logs for intelligence agencies."
        ]
    },
    "general_cyber_crime": {
        "label": "Uncategorized / Emerging Cyber Offence",
        "scope": "Citizen",
        "bns": ["318", "319", "351"],
        "it_act": ["43", "66", "66D"],
        "emergency_steps": [
            "Preserve all primary electronic evidence under Section 63 BSA 2023 (unedited chats, logs, timestamps).",
            "Call 1930 immediately if financial loss or unauthorized fund diversion is detected.",
            "Lodge formal FIR with local Cyber Crime Police Station.",
            "Do not communicate with or pay the suspect."
        ]
    }
}
...
def parse_citizen_incident_nlp(text: str) -> dict:
    """
    Natural Language Parser for Citizen Cyber Incidents (Hinglish/Hindi/English).
    Extracts financial loss amounts, crime categories, platform, suspect entities,
    and returns structured incident intelligence for automated FIR drafting.
    """
    ...
    # 1. Amount Extraction (Lakhs, Thousands, K, Hazar, direct figures)
    extracted_amount = None
    extracted_amount_raw = None
    
    lakh_match = re.search(r'(?:₹|rs\.?|inr)?\s*(\d+(?:\.\d+)?)\s*(?:lakh|lac|laakh)\b', lower)
    if lakh_match:
        val = int(round(float(lakh_match.group(1)) * 100000))
        extracted_amount = f"{val:,}"
        extracted_amount_raw = val
    else:
        k_match = re.search(r'(?:₹|rs\.?|inr)?\s*(\d+(?:\.\d+)?)\s*(?:k|hazar|hazaar|thousand)\b', lower)
        if k_match:
            val = int(round(float(k_match.group(1)) * 1000))
            extracted_amount = f"{val:,}"
            extracted_amount_raw = val
        else:
            num_match = (
                re.search(r'(?:₹|rs\.?|inr)\s*(\d{1,3}(?:,\d{2,3})*(?:\.\d+)?|\d{3,9})\b', text, re.IGNORECASE) or
                re.search(r'(\d{1,3}(?:,\d{2,3})*(?:\.\d+)?|\d{3,9})\s*(?:₹|rs\.?|inr|rupees|rupaye|\/-)\b', text, re.IGNORECASE) or
                re.search(r'(\d{1,3}(?:,\d{2,3})*(?:\.\d+)?|\d{3,9})\s*(?:cut|nikal|gaye|chale gaye|debit|lost|transfer|le liye|pay kiye|fraud|mang rahe|maang rahe|maanga|invest)\b', text, re.IGNORECASE) or
                re.search(r'(?:cut|nikal|gaye|chale gaye|debit|lost|transfer|le liye|le gaya|pay kiye|fraud|gawae|mang rahe|maang rahe|demand|demanded|invest|invested)\s*(?:of|rs\.?|₹)?\s*(\d{1,3}(?:,\d{2,3})*(?:\.\d+)?|\d{3,9})\b', text, re.IGNORECASE) or
                re.search(r'\b(\d{4,8})\b', text)
            )
            if num_match:
                raw_num = num_match.group(1).replace(',', '')
                val = int(float(raw_num))
                if 0 < val and not (6000000000 <= val <= 9999999999):
                    extracted_amount = f"{val:,}"
                    extracted_amount_raw = val
                    
    # 2. Comprehensive Crime Category Classification
    category = "general_cyber_crime"
    
    if (re.search(r'\b(child|minor|pocso|bachha|bachhe|underage|school kid|chota bacha)\b.*?\b(nude|porn|abuse|bad touch|molest|exploit|objectionable|compromised|indecent)\b', lower) or
        re.search(r'\b(nude|porn|abuse|bad touch|exploit|objectionable|compromised|indecent)\b.*?\b(child|minor|pocso|bachha|bachhe|underage)\b', lower) or
        re.search(r'\b(csam|child porn)\b', lower)):
        category = "csam_pocso_offence"
    elif re.search(r'\b(loan app|instant loan|recovery agent|kredit|contacts ko phone|contacts ko call|contacts ko message|gallery access|gallery leak|dhamki de rahe|gali de rahe|overdue threat|pan card pe loan|chinese loan|loan repayment|recovery harassment)\b', lower):
        category = "loan_app_fraud"
    elif re.search(r'\b(deepfake|morph|morphed|face swap|face replace|nude edit|fake nude|photo edit karke|chehra badal|ai nude|ai generated photo|compromised photo)\b', lower):
        category = "deepfake_morphing_harassment"
    elif re.search(r'\b(sextortion|nude|blackmail|viral|intimate|obscene|private video|video call recording|kapde utar|nangi video|video call pe nude)\b', lower):
        category = "sextortion"
    elif re.search(r'\b(electricity|bijli|bijli bill|power cut|line kat|light cut|bill update|power disconnected|gas bill|water bill|meter disconnect|challan apk|echallan|e-challan|traffic fine apk)\b', lower):
        category = "utility_bill_fraud"
    elif re.search(r'\b(shaadi\.com|jeevansathi|matrimony|matrimonial|tinder|bumble|dating app|honeytrap|nri groom|foreign friend|uk friend|gift parcel airport|marriage fraud)\b', lower):
        category = "matrimonial_romance_fraud"
    elif re.search(r'\b(fedex|dhl|courier|customs|customs officer|narcotics|mdma|drugs parcel|parcel me drugs|parcel seize|mumbai airport parcel|contraband|customs clearance fee)\b', lower):
        category = "courier_customs_fraud"
    elif re.search(r'\b(digital arrest|fake police|cbi|ed officer|enforcement directorate|crime branch|court hearing video|money laundering arrest|dcp cyber|supreme court notice|arrest warrant|trai notice)\b', lower):
        category = "digital_arrest"
    elif re.search(r'\b(atm clone|card clone|cloned card|skimming|card skim|pos machine fraud|unauthorized atm withdrawal|atm se paise nikal|card swipe fraud|debit card clone|credit card clone)\b', lower):
        category = "card_skimming_pos_fraud"
    elif re.search(r'\b(olx|quikr|army officer|fauji|cantonment|military vehicle|qr code scan to receive|scan karke paise aayenge|qr scan receive|delivery fee fraud|advance olx)\b', lower):
        category = "ecommerce_olx_fraud"
    elif re.search(r'\b(job\b.*?\b(offer|vacancy|visa|letter|fee)|visa\b.*?\b(fraud|fee|permit|offer|job)|fake job|work permit|interview fee|appointment letter|placement fraud|abroad job|overseas job|embassy fee)\b', lower):
        category = "job_visa_fraud"
    elif re.search(r'\b(telegram.*?(task|like|earn|commission|invest)|task.*?(telegram|wfh|part time)|part time|youtube.*?(like|task)|subscribe task|hotel review|rating task|prepaid task|commission scam|wfh task|ghar baithe kamaye|google review task)\b', lower):
        category = "part_time_job"
    elif re.search(r'\b(lottery|kbc|kaun banega crorepati|lucky draw|car prize|gift voucher|prize money|25 lakh lottery|processing fee for prize|unclaimed reward)\b', lower):
        category = "lottery_reward_fraud"
    elif re.search(r'\b(stalking|stalker|harass|harassment|doxx|doxxing|fake instagram profile|abusive messages|piche pada hai|gande message|threaten to defame|trolling|character assassination)\b', lower):
        category = "cyber_stalking_harassment"
    elif re.search(r'\b(cyber terror|critical infrastructure|power grid hack|scada|sovereignty|national security cyber)\b', lower):
        category = "cyber_terrorism_sabotage"
    elif re.search(r'\b(ransomware|locked files|\.locked|decryptor|ransom note|server encrypted|backup deleted)\b', lower):
        category = "corporate_ransomware"
    elif re.search(r'\b(ceo fraud|wire transfer|invoice change|vendor payment changed|business email compromise|beneficiary bank change|spoofed email transfer)\b', lower):
        category = "corporate_bec"
    elif re.search(r'\b(vendor invoice|vendor redirection|spoofed vendor|supplier bank change)\b', lower):
        category = "corporate_vendor_fraud"
    elif re.search(r'\b(insider theft|source code stolen|ex-employee data|trade secret|client list stolen)\b', lower):
        category = "corporate_insider_theft"
    elif re.search(r'\b(brand impersonation|typosquatting|fake domain of company|trademark phishing)\b', lower):
        category = "corporate_impersonation"
    elif re.search(r'\b(crypto|bitcoin|forex|stock|trading app|profit guarantee|double paisa|investment scheme|share market group|ipo allotment|sebi group|daily return|pig butchering)\b', lower):
        category = "investment_fraud"
    elif re.search(r'\b(sim swap|esim|aadhaar biometric|pan card misuse|clone sim|unauthorized sim|identity theft|aadhar misused|voter id misused)\b', lower):
        category = "identity_theft"
    elif re.search(r'\b(apk|download|install|phishing link|fake link|update pan|kyc blocked|link pe click|anydesk|teamviewer|rustdesk|quicksupport|screen share)\b', lower):
        category = "phishing"
    elif re.search(r'\b(hacked|account hack|instagram hacked|facebook hacked|email hacked|malware|trojan|keylogger|spyware|unauthorized login)\b', lower):
        category = "hacking"
    elif re.search(r'\b(upi|gpay|phonepe|paytm|unauthorized debit|bank cut|paise cut gaye|bank se nikal gaye|paise nikal gaye|unauthorized transaction|money debited)\b', lower):
        category = "upi_fraud"
    else:
        if extracted_amount_raw and extracted_amount_raw > 0:
            category = "upi_fraud"
        elif re.search(r'\b(threat|dhamki|fear|intimidate|abuse|blackmail|police|court|arrest)\b', lower):
            category = "general_cyber_crime"
        elif re.search(r'\b(login|access|account|device|phone|computer|system|password)\b', lower):
            category = "hacking"
        else:
            category = "general_cyber_crime"
    ...
    if "telegram" in lower:
        platform = "Telegram Application"
    elif "skype" in lower or "video call" in lower:
        platform = "Skype / Video Conference"
    elif "sms" in lower or "message" in lower:
        platform = "SMS (Smishing Link)"
    elif "call" in lower or "phone" in lower:
        platform = "Direct Cellular Call"
    elif "apk" in lower or "app" in lower or "link" in lower:
        platform = "Phishing Website / Malicious APK"
    elif "email" in lower or "mail" in lower:
        platform = "Spoofed Email Communication"
    elif "instagram" in lower or "insta" in lower:
        platform = "Instagram"
    elif "facebook" in lower or "fb" in lower:
        platform = "Facebook"
    elif "olx" in lower or "quikr" in lower:
        platform = "Online Classifieds (OLX/Quikr)"
        
    entities = extract_scam_entities(text)
    
    label = CRIME_LEGAL_MAP.get(category, {}).get("label", "Cyber Crime Incident")
    ...
    return {
        ...
        "amount_raw": extracted_amount_raw,
        "platform": platform,
        "entities": entities
    }
...
# ==============================================================================
# V2 ENHANCEMENT: GOVERNMENT NUMBER MATCHING & IT SECURITY ALERT SYSTEM
# ==============================================================================

GOV_DIRECTORY_CSV = "government_directory_numbers.csv"
GOV_ALERTS_CSV = "government_it_security_alerts.csv"

class GovernmentNumberMatcher:
    """
    Scans and cross-references suspect telephone numbers against official
    Government of India departments, state electricity utilities (DISCOMs),
    railways, police, and tax enforcement agencies to detect caller-ID spoofing
    and device hijacking.
    """
    def __init__(self, dir_csv=GOV_DIRECTORY_CSV, alerts_csv=GOV_ALERTS_CSV):
    ...
    def load_directory(self):
        ...
        if not os.path.exists(self.dir_csv):
            ...
            return
        try:
            with open(self.dir_csv, mode="r", encoding="utf-8") as f:
                reader = csv.DictReader(f)
                for row in reader:
                    self.directory.append(row)
        except Exception as e:
            print(f"[-] Warning: Failed to load {self.dir_csv}: {e}")

    def check_number(self, suspect_phone: str) -> dict or None:
        norm = self.normalize_phone(suspect_phone)
        if not norm or len(norm) < 10:
            return None
        for item in self.directory:
            official_norm = self.normalize_phone(item.get("official_phone", ""))
            if official_norm == norm or (len(official_norm) >= 10 and norm in official_norm):
                return item
        return None

    def dispatch_security_alert(self, matched_entry: dict, attack_type: str = "Caller ID Spoofing / Impersonation") -> dict:
        alert_id = f"ALT-SEC-2026-{secrets.randbelow(9000) + 1000}"
        now_ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        alert_record = {
            "alert_id": alert_id,
            "timestamp": now_ts,
            "department": matched_entry.get("department_name", "Unknown Dept"),
            "officer_designation": matched_entry.get("official_name_or_post", "Unknown Post"),
            "compromised_number": matched_entry.get("official_phone", ""),
            "attack_type": attack_type,
            "it_security_email": matched_entry.get("it_security_email", ""),
            "status": "DISPATCHED",
            "recommended_action": "Immediate SIM isolation with TSP; audit SIP trunk routing; broadcast consumer alert"
        }

        # Append to CSV
        file_exists = os.path.exists(self.alerts_csv)
        try:
            with open(self.alerts_csv, mode="a", encoding="utf-8", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=[
                    "alert_id", "timestamp", "department", "officer_designation",
                    "compromised_number", "attack_type", "it_security_email",
    ...
    def generate_dispatch_notice(self, matched_entry: dict, alert_record: dict, complainant: str = "Aggrieved Citizen") -> str:
        return f"""================================================================================
STATUTORY CYBER INCIDENT DISPATCH: COMPROMISED / SPOOFED GOVERNMENT TELEPHONE
Pursuant to Section 70B Information Technology Act, 2000 & CERT-In Protocols
================================================================================
DISPATCH NOTICE ID    : {alert_record['alert_id']}
TIMESTAMP             : {alert_record['timestamp']} (IST)
SEVERITY LEVEL        : HIGH / CRITICAL THREAT
TARGET DEPARTMENT     : {matched_entry.get('department_name')}
DIVISION / BOARD      : {matched_entry.get('division_or_board')}
OFFICIAL DESIGNATION  : {matched_entry.get('official_name_or_post')}
OFFICIAL PHONE NUMBER : {matched_entry.get('official_phone')}
SOC RECIPIENT EMAIL   : {matched_entry.get('it_security_email')}
COMPLAINANT CITIZEN   : {complainant}

INCIDENT SUMMARY:
An aggrieved citizen has filed a formal cybercrime report alleging fraudulent
solicitation where perpetrators actively utilized or spoofed the official telephone
number registered to your department ({matched_entry.get('official_phone')}).

URGENT CONTAINMENT PROTOCOLS REQUESTED FROM DEPARTMENT SOC:
1. Requisition signaling and SIP trunk routing logs from the Telecom Service Provider.
2. Confirm device integrity of the assigned official to rule out SIM-swap or spyware.
3. Broadcast consumer warning clarifying that official officers never demand UPI payments.
4. Preserve Call Detail Records (CDR) under Section 94 BNSS 2023 for Cyber Police inspection.
================================================================================"""

gov_matcher = GovernmentNumberMatcher()


# ==============================================================================
# V2 ENHANCEMENT: TRIAL PERSONALIZED QUESTIONNAIRE ENGINE
# ==============================================================================

class PersonalizedCrimeEngine:
    """
    Trial feature: Dynamically presents specialized investigative prompts
    for non-financial cyber offences (Digital Arrest, Sextortion, Predatory Loans,
    Deepfakes, Customs Narcotics scams, Ransomware).
    """
    QUESTIONS = {
        "digital_arrest": [
            ("Agency Impersonated (CBI / ED / Police / Customs)", "CBI / Cyber Crime Branch"),
            ("Video Call Medium (Skype / WhatsApp Video / Zoom)", "Skype Video Call"),
            ("Fabricated Allegation (MDMA in parcel / Money Laundering)", "Parcel intercepted with MDMA"),
            ("Demands Made (24/7 Digital Custody / Room Search / Escrow Transfer)", "Forced 24/7 camera surveillance"),
            ("Bogus Officer Name & Fake Badge ID shown", "DCP Vikram Singh, Badge #DL-8902")
        ],
        "sextortion": [
            ("Platform Where Contact Began (WhatsApp / Instagram / Dating App)", "WhatsApp Video Call"),
            ("Nature of Material (Recorded call snippet / AI morphed nude)", "Screen recorded video call"),
            ("Threatened Audience (Family WhatsApp group / Friends / YouTube)", "Threatened to send to family & contacts"),
            ("Follow-up by Fake Police (Fake Cyber Cell Inspector call?)", "Yes, received fake Cyber Cell call")
        ],
        "loan_app_fraud": [
            ("Instant Loan App Name & APK Source", "FastRupee (APK via SMS link)"),
            ("Permissions Extorted (Contacts / Photos / SMS)", "Full Contact Address Book & Photos"),
            ("Harassment Target Audience (Calls to parents / Defamatory group)", "Abusive WhatsApp group with contacts"),
            ("Principal Disbursed vs Extortionate Demand", "Rs. 3,500 disbursed, Rs. 14,000 demanded")
        ],
        "deepfake_morphing_harassment": [
            ("Synthetic Media Type (AI face swap / Cloned voice / Deepfake video)", "AI Face-Swap Deepfake Video"),
            ("Hosting Platform & Offending URLs", "Instagram profile / Telegram channel"),
            ("Rule 3(2)(b) Notice Sent to Platform Grievance Officer?", "Not yet sent - Request takedown in FIR")
        ],
        "courier_customs_fraud": [
            ("Courier Service Impersonated (FedEx / DHL / India Post)", "FedEx Express"),
            ("Alleged Route & Illegal Contraband", "Mumbai to Taiwan with MDMA and forged passports"),
            ("Bogus AWB / Tracking Consignment ID", "FX-IND-98765432-BOM")
        ],
        "corporate_ransomware": [
            ("Compromised Infrastructure (Domain Controller / Backup NAS / DB)", "Active Directory & NAS Backup"),
            ("Ransomware File Extension & Note Filename", ".locked / README_RECOVER.txt"),
            ("CERT-In 6-Hour Incident Notification ID", "In Progress / CERTIN-2026-9812")
        ]
    }

    @classmethod
    def run_interactive_trial(cls, crime_key: str) -> dict:
        questions = cls.QUESTIONS.get(crime_key)
        if not questions:
            return {}
        print("\n" + "-"*70)
        print("  🧪 TRIAL FEATURE: PERSONALIZED CRIME QUESTIONNAIRE MATRIX")
        print("  Standard fields are valid for all crimes. Answer these specialized")
        print("  forensic questions to fortify your Court-Admissible Police FIR:")
        print("-"*70)
        answers = {}
        for prompt, default_val in questions:
            resp = input(f"  • {prompt} [{default_val}]: ").strip()
            answers[prompt] = resp if resp else default_val
        return answers

    @classmethod
    def format_particulars(cls, answers: dict) -> str:
        if not answers:
            return ""
        lines = ["\n[STATUTORY FORENSIC EVIDENCE PARTICULARS - TRIAL QUESTIONNAIRE]"]
...
def display_menu():
    ...
    print(" 12. 🏛️ Government Number Spoofing & IT Security Alert Desk (V2)")
    print(" 13. 🚪 Exit")
    print("\nEnter your choice (1-13): ", end="")
...
def run_gov_verification_menu():
    while True:
        print("\n" + "="*75)
        print("  🏛️ GOVERNMENT NUMBER VERIFICATION & IT SECURITY ALERT DESK (V2)")
        print("="*75)
        print("  1. Check Suspect Phone Number against Official Government Directory")
        print("  2. View Recent Dispatched IT Security Alerts (government_it_security_alerts.csv)")
        print("  3. List All Monitored Government Departments (Railways, DISCOMs, Police, etc.)")
        print("  4. Add New Official Government Number to Mock Directory")
        print("  5. Return to Main Menu")
        choice = input("\nEnter option (1-5): ").strip()

        if choice == "1":
            phone = input("Enter suspect phone number: ").strip()
            matched = gov_matcher.check_number(phone)
            if matched:
                print("\n" + "!"*75)
                print("🚨 CRITICAL WARNING: COMPROMISED / SPOOFED GOVERNMENT TELEPHONE DETECTED")
                print(f"   Suspect Phone    : {phone}")
                print(f"   Official Post    : {matched.get('official_name_or_post')}")
                print(f"   Department       : {matched.get('department_name')}")
                print(f"   Division / Board : {matched.get('division_or_board')}")
                print(f"   Official Phone   : {matched.get('official_phone')}")
                print(f"   Official Email   : {matched.get('official_email')}")
                print(f"   IT Security SOC  : {matched.get('it_security_email')}")
                print("!"*75)
                print("⚠️  ADVISORY: Government agencies and police NEVER issue digital arrest warrants")
                print("   over video calls, nor do electricity DISCOMs demand bill payment to personal UPI VPAs.")
                dispatch_now = input("\nDispatch statutory IT Security alert notice to department SOC? (y/n) [y]: ").strip().lower()
                if dispatch_now != 'n':
                    alert = gov_matcher.dispatch_security_alert(matched)
                    notice = gov_matcher.generate_dispatch_notice(matched, alert)
                    print(f"\n[✔] Incident Alert {alert['alert_id']} logged into {GOV_ALERTS_CSV}")
                    print(notice)
            else:
                print(f"\n[i] Phone number '{phone}' did not match any official government department in directory.")

        elif choice == "2":
            print(f"\n--- DISPATCHED IT SECURITY ALERTS ({GOV_ALERTS_CSV}) ---")
            if not os.path.exists(GOV_ALERTS_CSV):
                print("No alerts recorded yet.")
                continue
            with open(GOV_ALERTS_CSV, mode="r", encoding="utf-8") as f:
                reader = csv.reader(f)
                rows = list(reader)
            if len(rows) <= 1:
                print("No alerts recorded yet.")
            ...
            print(f"\n--- MONITORED GOVERNMENT DIRECTORY ({len(gov_matcher.directory)} Records) ---")
            for item in gov_matcher.directory:
                print(f"• [{item.get('dept_id')}] {item.get('official_phone'):<15} | {item.get('official_name_or_post')} ({item.get('department_name')})")

        elif choice == "4":
            dept = input("Department Name (e.g. Northern Railway): ").strip()
            div = input("Division / Board: ").strip()
            post = input("Official Designation / Post: ").strip()
            phone = input("Official Phone Number: ").strip()
            email = input("Official Email: ").strip() or "officer@gov.in"
            sec_email = input("IT Security SOC Email: ").strip() or "soc@gov.in"
            if dept and post and phone:
                new_row = {
                    "dept_id": f"GOV-{secrets.randbelow(900)+100}",
                    "department_name": dept,
                    "division_or_board": div or dept,
                    "official_name_or_post": post,
                    "official_phone": phone if phone.startswith("+") else "+91-" + phone,
                    "official_email": email,
                    "it_security_email": sec_email,
                    "authorized_role": "Official Communication",
                    "status": "ACTIVE"
                }
                gov_matcher.directory.append(new_row)
                file_exists = os.path.exists(GOV_DIRECTORY_CSV)
                with open(GOV_DIRECTORY_CSV, mode="a", encoding="utf-8", newline="") as f:
                    writer = csv.DictWriter(f, fieldnames=[
                        "dept_id", "department_name", "division_or_board", "official_name_or_post",
                        "official_phone", "official_email", "it_security_email", "authorized_role", "status"
                    ])
                    if not file_exists:
                        writer.writeheader()
                    writer.writerow(new_row)
                print(f"\n[✔] Added {phone} ({dept}) to {GOV_DIRECTORY_CSV}.")
